...
public class PlayerTestFunctions extends AdminCommand
{
	...
	protected void handleUpdate(Player player, String... params) {
		...
		if (COMMAND_PROTECTOR.equalsIgnoreCase(params[0])) {
			...
			/*for (PlayerCPEntry a : player.getCP().getAllCP()) {
				if (a.getSlot() == 1) {
					PacketSendUtility.sendMessage(player, "STATS 1 "+ a.getPoint());
				} if (a.getSlot() == 2) {
					PacketSendUtility.sendMessage(player, "STATS 2 "+ a.getPoint());
				} if (a.getSlot() == 3) {
