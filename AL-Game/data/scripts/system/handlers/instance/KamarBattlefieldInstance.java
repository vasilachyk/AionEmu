...
激戦地カマルは12 vs 12の戦場！
12 vs 12の合計24人が参加した戦場で、激戦地ドレドギオンの12人バージョンともいえる場所となる。
相手陣営を撃退、ネームドの暗殺、スコアオブジェクト取得などを通じてポイントを獲得することができ、最終的なスコアが高い陣営が勝利となる。

- 12 VS 12、合計24人が参加できる戦場。
- 相手陣営を撃退、ネームド暗殺、スコアオブジェクト取得などを通じてポイントを獲得することができる。
- 制限時間30分経過後、最終的なスコアが高い陣営が勝利する。
- 日、月、水、土曜日の午後8時～10時に各曜日1回
- レベル61～65で種族ごとに12名ずつ入場可能

背景ストーリー
数々の逆境を乗り越えた煉族の輝かしい歴史を代弁してきた花の都カマルは、カルンが失踪した後に状況が急激に変化した。
束縛の強さはゆるくなり、天族と魔族はカマルに一時的に指揮官を残して少しでも隙が見えたら、お互いを除去し、カマロを得ようとする野心を見せた。
煉族、特にカルノンは、千魔族の緊張状態をおさえるしようとしたが、それらを拘束する力が弱くなったので、もはや通用しなかった。
 
一方、このようなカマルの事情を看破したベリトラは、部隊をカマルに投入するよう指示した。
その部隊はカマルで最もよく知られているドレドギオン軍団長であった。このような龍族の陰謀と来るべきリスクを知らずに魔族と天族は、それぞれの立場に真っ向から対立した。
その中、襲撃を開始した竜族は、突撃隊司令官ヴァルガの指揮の下、ついにカマル入りに成功する。
侵入されたカマルは、これまでの平和が無情にも燃え上がり凄惨な戦場に変わってしまう。

入場方法
- 定められた時間の間だけ入場することができる。
- 決まった時間になると、ドレドギオンと同様に、画面の右下に入場ボタンが有効になる。
- パーティーやフォースを事前に設定して入場申請をすることも可能であり、個人的での参加ももちろん可能である。
